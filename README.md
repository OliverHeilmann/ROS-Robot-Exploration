...
Readers are welcome to jump around the Readme to sections of interest. The project is structured as follows:
1. [Transformations](#1-transformations)
2. [OpenCV and Object Tracking](#2-opencv-and-object-tracking)
3. [Object Detection using YoloV5](#3-object-detection-using-yolov5)
4. [SLAM and AMCL (Adaptive Monte-Carlo Localisation)](#4-slam-and-amcl-adaptive-monte-carlo-localisation)
5. [Navigation](#5-navigation)
6. [Exploration](#6-exploration)
7. [Joystick Robot Control](#7-joystick-robot-control)
8. [Clearpath Robotics](#8-clearpath-robotics)
...
This project demonstrates how to use the YoloV5 object detection model to detect objects in a video stream. The `cv_bridge` package is used to convert ROS messages to OpenCV images and vice versa. The YoloV5 model is then used to determine the new position of the object. Futher improvements to performance could be made with a CUDA enabled GPU (which I do not have currently). This section was created from scratch based on the information gained from the previous section.

To run the object detection model, use the following command:

```sh
# Terminal 1: Launch the object detection node
ros2 launch rosbot detection.launch.py

# Terminal 2: Launch the Gazebo simulation
ROSBOT_SIM
```

The object detection model is based on the YoloV5 model. The model is trained on the COCO dataset and is able to detect 80 different classes of objects. The model is able to detect objects in real-time. Unfortunately however, there are not many objects in the scene which is loaded that are detectable. More objects could be added to the scene to test the model further.

![RViz Screenshot 1](images/ObjectDetection.png)


## 4) SLAM and AMCL (Adaptive Monte-Carlo Localisation)
